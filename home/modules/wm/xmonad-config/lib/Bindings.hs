...
setup c =
  addDescrKeys'
    ...
    $ c
      { modMask = myModMask
      , manageHook = setHelpGeometry <+> manageHook c
      , logHook = closeHelpOnFocusLost <+> logHook c
      }
  where
    ...
    closeHelpOnFocusLost =
      withUnfocused $ \w -> runQuery query w --> withDisplay (io . void . (`killClient` w))
  ...
  XConfig
    ...
    foldl
      ...
      [ keySet
          ...
          , key "Terminal" (modm .|. shiftMask, xK_Return) $ spawn term
          , key "Emoji"    (modm .|. shiftMask, xK_e)      $ spawn "rofimoji"
          ]
      , keySet
          "Layouts"
          [ key "Next layout"          (modm, xK_space)               $ sendMessage NextLayout
          , key "Reset to base layout" (modm .|. shiftMask, xK_space) $ setLayout l
